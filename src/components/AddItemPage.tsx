...
import { PlusCircle, Image, MapPin, DollarSign, Tag, FileText, CheckCircle2, ArrowLeft, Upload, Trash2 } from 'lucide-react';
...
export const AddItemPage: React.FC<AddItemPageProps> = ({
  ...
  const [category, setCategory] = useState<ItemCategory>(editingItem?.category || 'Calculators');
  const [description, setDescription] = useState(editingItem?.description || '');
  const [rentPricePerDay, setRentPricePerDay] = useState(editingItem?.rentPricePerDay ? String(editingItem.rentPricePerDay) : '5');
  const [securityDeposit, setSecurityDeposit] = useState(editingItem?.securityDeposit ? String(editingItem.securityDeposit) : '20');
  const [condition, setCondition] = useState<ItemCondition>(editingItem?.condition || 'Good');
  const [pickupLocation, setPickupLocation] = useState(editingItem?.pickupLocation || 'CS Lab 3, 2nd Floor Engineering Building');
  
  const [images, setImages] = useState<string[]>(
    editingItem?.images || ['https://images.unsplash.com/photo-1587145820266-a5951ee6f620?auto=format&fit=crop&q=80&w=800']
  );
  ...
  const presetImages = [
    { label: 'TI Graphing Calculator', url: 'https://images.unsplash.com/photo-1587145820266-a5951ee6f620?auto=format&fit=crop&q=80&w=800' },
    { label: 'Raspberry Pi 4 Board', url: 'https://images.unsplash.com/photo-1629654297299-c8506221ca97?auto=format&fit=crop&q=80&w=800' },
    { label: 'Algorithms CS Textbook', url: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&q=80&w=800' },
    { label: 'Digital Oscilloscope Lab', url: 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&q=80&w=800' },
    { label: 'Arduino Circuit Board', url: 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&q=80&w=800' }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        category,
        description,
        images: images.length > 0 ? images : ['https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&q=80&w=800'],
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Share equipment, books, or lab components with fellow Computer Science students
            </p>
          </div>
          <button
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              <span className="text-[10px] text-slate-400 font-semibold self-center mr-1">Sample Photos:</span>
              {presetImages.map((p) => (
                <button
                  key={p.label}
                  type="button"
          ...
          <div className="pt-4 flex items-center justify-end space-x-3">
            ...
            >
              ...
              <span>{editingItem ? 'Update Listing' : 'Publish Resource Listing'}</span>
            </button>
          </div>
